...
**Tab: Financial Spread** (portfolio: Walmart, AutoWest Motors, Coastal Hyundai, Meridian Foods)

This is the deepest proof of the two board tenets — **Agents operate, Trust is measurable** — on the actual spreading surface where competitors compete (S&P ProSpread, Moody's CreditLens, Evalueserve Spreadsmart). One engine, one chart of accounts, four real borrowers spanning all four lending scenarios in our Credit Policy: Term Loan B (Walmart), Floor Plan (AutoWest — distressed), Annual Review (Coastal Hyundai — healthy), and a fourth Term Loan B build (Meridian).

| Step | Action | Talk track |
|------|--------|------------|
| 1 | Click **Financial Spread** → land on **Walmart Inc.** | "One engine, one **master financial database** — not a one-off demo screen. Every statement we ingest — any quarter, any year, any borrower — is standardised by the Mapping Agent to the ACOS chart of accounts. This is the 'compare like-for-like' grid S&P and Moody's sell — but watch what we put on top of it." |
| 1b | Click **AutoWest Motors** in the Portfolio switcher, then **Coastal Hyundai** | "Same engine, different borrowers, different stories. AutoWest is a distressed floor-plan dealer — DSCR 0.00x, negative equity, Health grade E. Coastal Hyundai is a clean annual re-certification — Health grade B, zero exceptions. Four lending scenarios, one platform, all tied to our Credit Policy §2 case definitions." |
| 2 | Back to **Walmart**, point to the **period columns** (FY2023–FY2025) | "Side-by-side comparison, exactly like Capital IQ or a Yahoo Finance grid. Income statement, balance sheet, cash flow — all three, all periods, one source of truth." |
...
| 5 | Point to the **red integrity banner** — "Balance sheet balances: off by \$174.5B" | "Here's the difference. The platform runs **cross-statement integrity checks live** — does the balance sheet balance, does cash flow tie to cash, does net income tie across statements. It caught a discrepancy the extraction agent introduced. Moody's and S&P give you a spread; we tell you whether the spread is internally consistent." |
| 6 | In the grid, point to **Property, Plant & Equipment FY2025** in red (flagged) | "The Mapping Agent flagged one cell at low confidence — an OCR scale error, two orders of magnitude. Every other number ties. The agent didn't hide it or average it away; it surfaced the one thing that needs a human." |
| 7 | Click the PP&E cell → **Lineage strip** opens | "Click any value and it traces to its exact **page and source cell** on Walmart's own rendered Balance Sheet. That's cell-level lineage, not 'page 43' — and this is the same real source document that used to be an empty placeholder." |
| 8 | Enter the corrected value + a **rationale**, click **Save correction** | "I correct it in place with a documented reason. Watch: the subtotals, totals, ratios, and the integrity banner **recompute live** — the balance sheet re-balances, the exception clears, and the **Trust score jumps**. The edit is written to the case lifecycle with who/when/why." |
...
| How do you say no? | "Gate 5 isn't just Approve — the committee can Decline or Table a decision, per our credit committee charter (Credit Policy §14). Decline closes the case with a written rationale on the record; Table returns it for more information. It's not a rubber stamp." |
| Who's actually allowed to approve what? | "Credit Policy §13 — an authority matrix by exposure. An analyst signs Gate 1–2 on ordinary cases; anything with an open Critical exception, or over $25M, escalates to a Senior Analyst or Risk Officer; anything over $100M — or any Floor Plan facility over $10M — goes to committee. It's not one person signing every gate on a $750M facility." |
