...
**Where this leaves things:** the entire Command Center → Cases → 5-gate workflow (case creation, document intake, gate sign-off, the "Total Assets" mapping correction, audit trail) is now backend-real for Walmart and Northern Retail. What's still local/fixture-only: mapping fields other than "Total Assets", Gate 5 decline/table, portfolio widgets for the other borrowers (AutoWest/Costco/Target — still routed to Walmart per the documented shortcut), and the Financial Spread tab (separate subsystem, out of scope for this phase per docs/ROADMAP.md).
